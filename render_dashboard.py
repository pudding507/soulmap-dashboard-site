...
SECTIONS = [
 ...
 ("⑤ 对话 · Chat", [
   ...
   ("chat_engaged_new_user_rate", "新用户投入率 · Engaged New-User Rate", "rate",
       dict(rate=("engaged_new_users","new_users"),
            note="当天注册的新用户里,当天就发出≥5条消息的占比 ｜ Share of each day's new users who sent ≥5 messages on day 0",
            dims=[("overall","Overall",None)])),
   ("chat_engaged_user_focus", "投入用户数 · Engaged Users", "line",
       dict(val="users",
            note="当天发≥5条 user 消息的人数。purpose 是旧版 onboarding 表单字段(userPurpose),新版用 5 个 anchor(topicAnchor,07-19 起),两版并存 ｜ Users sending ≥5 messages that day. Purpose comes from the old onboarding form (userPurpose); the new form uses 5 anchors (topicAnchor, from 07-19) — both still in use",
            dims=[("overall","Overall",None),
 ...
 ("⑦ 发现 · Discover", [
   ...
   ("discover_scroll_depth", "滚动深度分布 · Scroll Depth Distribution", "line", dict(val="users", slfmt=(lambda s: str(int(float(s)))+"%"),
   ...
   ("discover_empty_state", "空状态表现 · Empty-State Rate", "line", dict(val="empty_users",
